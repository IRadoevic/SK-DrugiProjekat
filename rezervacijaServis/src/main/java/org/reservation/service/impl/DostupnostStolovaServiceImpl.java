...
public class DostupnostStolovaServiceImpl implements DostupnostStolovaService {
    ...
    public void rezervisi(RezervacijaDto rezervacijaDto) {
       ...
       Integer brRezervacija = Bulkhead.decorateSupplier(userServiceBulkhead, ()-> Retry.decorateSupplier(userServiceRetry, ()->getBrRezervacija(rezervacijaDto.getIdKorisnika())).get()).get();
        ...
        RestoranLoyalty restoranLoyalty = restoranLoyaltyRepository.findById(restoran.getId()).orElseThrow(() -> new NotFoundException("Restoran nema pogodnosti "));
        //sada proverimo jel ispunjava pogodnosti
        if(brRezervacija>=restoranLoyalty.getUslov())
        {
            // ispunjava uslov
            // pravimo loyalty ststus korisnika
            // menadyeru restorana saljemo mejl
            //saljemo useru da smanji ukupan broj rezervacija za uslov
        }
